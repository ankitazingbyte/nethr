...
class NoticesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create notice" do
    assert_difference('Notice.count') do
      post notices_url, params: { notice: { description: @notice.description, status: @notice.status, title: @notice.title } }
    end
  ...
  test "should update notice" do
    patch notice_url(@notice), params: { notice: { description: @notice.description, status: @notice.status, title: @notice.title } }
